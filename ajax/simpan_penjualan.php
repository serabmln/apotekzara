<?php 
 ...
 //    }
	...
	for($i = 0; $i < count($_POST['hidden_kdobat']); $i++) {
		...
		$profit = $_POST['hidden_jml_profit'][$i];
		$subtotal = $_POST['hidden_subtotal'][$i];
		$exp_obat = $_POST['hidden_expobat'][$i];

		$query_pjldtl = "INSERT INTO tbl_penjualandetail (no_penjualan, kd_obat, expired, hrg_jual, jml_jual, sat_jual, profit_obat, subtotal) VALUES('$no_penjualan', '$kd_obat', '$exp_obat', '$hrg_jual', '$jml_obat', '$sat_jual', '$profit', '$subtotal')";
		mysqli_query($conn, $query_pjldtl) or die ($conn->error);
