<?php 
	...
	if(@$_GET['page']=='dataobat')
	{
		...
		mysqli_query($conn, $query) or die ($conn->error);
		$query2 = "DELETE FROM tbl_stokexpobat WHERE kd_obat = '$id'";
		mysqli_query($conn, $query2) or die ($conn->error);
	}
	...
	if(@$_GET['page']=='datapenjualan')
	{
		...
		$query_pjl_detail = "DELETE FROM tbl_penjualandetail WHERE no_penjualan = '$id'";
		mysqli_query($conn, $query_pjl_detail) or die ($conn->error);
	}
