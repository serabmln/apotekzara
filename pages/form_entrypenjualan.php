                    ...
                    <div class="row kotak-form-tabel-penjualan">
                    	<div class="col-md-3 kotak-form-obat-terjual" style="display: ;">
                    		...
                    		<!-- <form action="javascript:void(0);">  -->
                                ...
                                <div class="position-relative form-group">
                                    <div class="input-group input-group-sm">
                                        <input type="hidden" name="jml_profit" id="jml_profit" placeholder="" type="number" class="form-control form-control-sm" disabled="">
                                    </div>
                                </div>
                                <div class="position-relative form-group">
                                    <div class="input-group input-group-sm">
                                        <input type="hidden" name="jml_total_profit" id="jml_total_profit" placeholder="" type="number" class="form-control form-control-sm" disabled="">
                                    </div>
                                </div>
                                <div class="position-relative form-group">
...
$(document).ready(function() {
    ...
    function calculateProfit() {
        var jml = Number($("#jml_obat").val()); // Ambil nilai jumlah obat
        var profitPerItem = Number($("#jml_profit").val()); // Ambil nilai profit per item

        if (jml >= 0 && profitPerItem >= 0) {
            var totalProfit = jml * profitPerItem; // Hitung total profit
            $("#jml_total_profit").val(totalProfit); // Set nilai total profit
        } else {
            $("#jml_total_profit").val(""); // Jika nilai tidak valid, kosongkan field
        }
    }

    // Jalankan fungsi setiap kali input diubah
    $("#jml_obat, #profit_per_item").on("input", calculateProfit);
    ...
    $("#tambah_obat").click(function() {
        ...
        var total_profit = $("#jml_total_profit").val();
        ...
        if(kode=="") {
            ...
            output += '  <td>' + kode +
                    '    <input type="hidden" name="hidden_kdobat[]" ' +
                    '           id="td_kd_obat' + count + '" ' +
                    '           class="td_kd_obat" value="' + kode + '">' +
                    '  </td>';
            output += '  <td>' + nama +
                    '    <input type="hidden" name="hidden_nmobat[]" ' +
                    '           id="td_nmobat' + count + '" ' +
                    '           class="td_nmobat" value="' + nama + '">' +
                    '  </td>';
            output += '  <td>' + exp +
                    '    <input type="hidden" name="hidden_expobat[]" ' +
                    '           id="td_expobat' + count + '" ' +
                    '           class="td_expobat" value="' + exp + '">' +
                    '  </td>';
            output += '  <td class="text-right">' + harga +
                    '    <input type="hidden" name="hidden_hrgobat[]" ' +
                    '           id="td_hrgobat' + count + '" ' +
                    '           class="td_hrgobat" value="' + harga + '">' +
                    '  </td>';
            output += '  <td class="text-center">' + jumlah +
                    '    <input type="hidden" name="hidden_jmlobat[]" ' +
                    '           id="td_jmlobat' + count + '" ' +
                    '           class="td_jmlobat" value="' + jumlah + '">' +
                    '  </td>';
            output += '  <td class="text-center">' + total_profit +
                    '    <input type="hidden" name="hidden_jml_profit[]" ' +
                    '           id="td_jmlprofit' + count + '" ' +
                    '           class="td_jmlprofit" value="' + total_profit + '">' +
                    '  </td>';
            output += '  <td class="text-center">' + satuan +
                    '    <input type="hidden" name="hidden_satobat[]" ' +
                    '           id="td_satobat' + count + '" ' +
                    '           class="td_satobat" value="' + satuan + '">' +
                    '  </td>';
            output += '  <td class="text-right">' + subtotal +
                    '    <input type="hidden" name="hidden_subtotal[]" ' +
                    '           id="td_subtotal' + count + '" ' +
                    '           class="td_subtotal" value="' + subtotal + '">' +
                    '  </td>';
            output += '  <td class="td-opsi">' +
                    '    <button type="button" class="hapus_obat btn-transition btn btn-outline-danger btn-sm" ' +
                    '            name="hapus_obat" id="' + count + '" title="Hapus obat ini">Hapus</button>' +
                    '  </td>';
            output += '</tr>';
